...
![image-20221009001106188](./chap19_router_ospfv3.assets/image-20221009001106188.png)
...
可以使用show ipv6 ospf neighbor来查看OSPFv3的邻居关系正常，如下所示，并且可知路由器R3是DR路由器，R2是BDR路由器，关于为什么这样选举，在OSPFv2中有详细描述，这里不再重复描述。然后可以通过show ipv6 route查看路由器R1的IPv6路由表，如下所示，可看出R1成功的学习到了路由器R2和R3公告出来的OSPF路由，其中的“Ｏ”就表示通过OSPFv3所学到的路由。最后在路由器R1上通过ping指令检测与路由器R2和R3上相关IPv6前缀的连通性，一切正常。

```bash
R1#show ipv6 ospf neighbor

Neighbor ID   Pri   State            Dead Time   Interface ID   Interface
3.3.3.3        1    FULL/DROTHER  00:00:36    6            GigabyteEthernet0/0/0
2.2.2.2        1    FULL/DR         00:00:33    6            GigabyteEthernet0/0/0
```
...
via FE80::6FE:7FFF:FEEB:7E9, GigabyteEthernet0/0/0
